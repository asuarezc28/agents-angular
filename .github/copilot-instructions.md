...
- **Tailwind CSS**: 3.x (`tailwindcss-primeui` integration)
...
### Chart Color Palette: colors.constants.ts

**CRITICAL:** `src/app/core/constants/colors.constants.ts` is currently used by `ThemeColorsService` for programmatic colors in specific charts/TS cases (por ahora).

1. **theme-colors.service.ts** - For programmatic access in TypeScript/ECharts
...
3. ThemeColorsService-based visuals update (specific charts/TS cases):

- ✅ Chart/TS color logic
- ✅ Reactive ThemeColorsService signals

**How it works:**
PrimeNG theme tokens come from official Aura in `app.config.ts`, while `tailwindcss-primeui`
aligns Tailwind utilities with PrimeNG styling.

### PrimeNG + Tailwind System

This project uses the official integration:

- `tailwindcss-primeui` in `tailwind.config.ts`
- PrimeNG official preset (`Aura`) in `app.config.ts`
...
- Edit color VALUES in `colors.constants.ts` for chart/TS palette needs
...
- Values come from `colors.constants.ts` (chart/TS palette)
...
- Components use CSS variables (`--p-*`) provided by PrimeNG theme tokens
- Theme tokens stay aligned with Tailwind via `tailwindcss-primeui`
...
- PrimeNG theme tokens + Tailwind utilities avoid hardcoded runtime color logic
