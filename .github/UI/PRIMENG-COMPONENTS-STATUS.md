...
**Sistema**: Integración oficial PrimeNG + Tailwind  
...
Estos componentes ya tienen todas sus variables CSS definidas en [tailwind.config.ts](../../tailwind.config.ts)
...
Cuando uses alguno de estos componentes en tu app, valida primero su comportamiento con el theme oficial y aplica overrides solo si son necesarios.
...
2. **Edita configuración de theme** - Agrega variables en `tailwind.config.ts`
...
- [tailwind.config.ts](../../tailwind.config.ts) - Plugin oficial tailwindcss-primeui
